...
<div class="bg-white rounded-xl shadow-lg border-l-4 border-[#7BAFD4]">
    ...
    <form action="{{ route('contratos.update', $contrato->id) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                <label for="cliente_id" class="block text-sm font-bold text-[#003262] mb-2">
            ...
            <div>
                ...
                <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $contrato->titulo) }}"
            ...
            <div>
                ...
                <select name="tipo" id="tipo"
                    ...
                    <option value="">Selecione</option>
                    <option value="Honorários" {{ old('tipo', $contrato->tipo) == 'Honorários' ? 'selected' : '' }}>Contrato de Honorários</option>
                    <option value="PrestacaoServicos" {{ old('tipo', $contrato->tipo) == 'PrestacaoServicos' ? 'selected' : '' }}>Contrato de Prestação de Serviços</option>
                    <option value="Exito" {{ old('tipo', $contrato->tipo) == 'Exito' ? 'selected' : '' }}>Contrato de Êxito (Quota Litis)</option>
                    <option value="Mensalidade" {{ old('tipo', $contrato->tipo) == 'Mensalidade' ? 'selected' : '' }}>Contrato de Mensalidade</option>
                    <option value="Consultoria" {{ old('tipo', $contrato->tipo) == 'Consultoria' ? 'selected' : '' }}>Contrato de Consultoria Jurídica</option>
                    <option value="Parceria" {{ old('tipo', $contrato->tipo) == 'Parceria' ? 'selected' : '' }}>Contrato de Parceria</option>
                    <option value="Assessoria" {{ old('tipo', $contrato->tipo) == 'Assessoria' ? 'selected' : '' }}>Contrato de Assessoria Jurídica</option>
                </select>
                @error('tipo')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
                ...
                <input type="date" name="data_assinatura" id="data_assinatura" value="{{ old('data_assinatura', $contrato->data_assinatura) }}"
                       class="w-full px-4 py-3 rounded-lg border-2 @error('data_assinatura') border-red-500 bg-red-50 @else border-[#9EB9D4] @enderror focus:border-[#20729E] focus:ring-2 focus:ring-[#20729E]/20 transition-all duration-200 outline-none">
            ...
            <div>
                <label for="data_vencimento" class="block text-sm font-bold text-[#003262] mb-2">
                    Data de Vencimento <span class="text-red-500">*</span>
                </label>
                <input type="date" name="data_vencimento" id="data_vencimento" value="{{ old('data_vencimento', $contrato->data_vencimento) }}"
                       class="w-full px-4 py-3 rounded-lg border-2 @error('data_vencimento') border-red-500 bg-red-50 @else border-[#9EB9D4] @enderror focus:border-[#20729E] focus:ring-2 focus:ring-[#20729E]/20 transition-all duration-200 outline-none">
            ...
            <div>
                <label for="valor" class="block text-sm font-bold text-[#003262] mb-2">
                    Valor <span class="text-red-500">*</span>
                </label>
                <input type="text" name="valor" id="valor" value="{{ old('valor', $contrato->valor) }}"
                       class="w-full px-4 py-3 rounded-lg border-2 @error('valor') border-red-500 bg-red-50 @else border-[#9EB9D4] @enderror focus:border-[#20729E] focus:ring-2 focus:ring-[#20729E]/20 transition-all duration-200 outline-none"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descricao" id="descricao" rows="3"
                          ...
                          placeholder="Descrição do contrato">{{ old('descricao', $contrato->descricao) }}</textarea>
                @error('descricao')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
