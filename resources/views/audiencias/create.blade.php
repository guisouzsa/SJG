...
@section('breadcrumb', 'Audiências / Nova')
    ...
    <div class="px-6 py-5 border-b border-[#9EB9D4]/30">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm mt-1 text-[#6699CC]">Preencha os dados da audiência</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="tipo" id="tipo"
                    ...
                    <option value="">Selecione</option>
                    <option value="Audiência de conciliação e mediação" {{ old('tipo') == 'Audiência de conciliação e mediação' ? 'selected' : '' }}>Audiência de conciliação e mediação</option>
                    <option value="Audiência de instrução e julgamento" {{ old('tipo') == 'Audiência de instrução e julgamento' ? 'selected' : '' }}>Audiência de instrução e julgamento</option>
                    <option value="Audiência de justificação" {{ old('tipo') == 'Audiência de justificação' ? 'selected' : '' }}>Audiência de justificação</option>
                    <option value="Audiência de custódia" {{ old('tipo') == 'Audiência de custódia' ? 'selected' : '' }}>Audiência de custódia</option>
                    <option value="Audiência una" {{ old('tipo') == 'Audiência una' ? 'selected' : '' }}>Audiência una</option>
                    <option value="Audiência de razões finais" {{ old('tipo') == 'Audiência de razões finais' ? 'selected' : '' }}>Audiência de razões finais</option>
                </select>
                @error('tipo')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
                ...
                <input type="datetime-local" name="data_horario" id="data_horario" value="{{ old('data_horario') }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descricao" id="descricao" rows="3"
                          ...
                          placeholder="Descrição da audiência">{{ old('descricao') }}</textarea>
                @error('descricao')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
