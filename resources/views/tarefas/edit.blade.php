...
@section('title', 'Editar Tarefa - SJG')
@section('breadcrumb', 'Tarefas / Editar')
...
<div class="bg-white rounded-xl shadow-lg border-l-4 border-[#7BAFD4]">
    ...
    <div class="px-6 py-5 border-b border-[#9EB9D4]/30">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-[#003262]">Editar Tarefa</h2>
                <p class="text-sm mt-1 text-[#6699CC]">Atualize os dados da tarefa</p>
            </div>
    ...
    <form action="{{ route('tarefas.update', $tarefa->id) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="data_limite" class="block text-sm font-bold text-[#003262] mb-2">
                    Data limite
                </label>
                <input type="date" name="data_limite" id="data_limite" value="{{ old('data_limite', $tarefa->data_limite) }}"
                       class="w-full px-4 py-3 rounded-lg border-2 @error('data_limite') border-red-500 bg-red-50 @else border-[#9EB9D4] @enderror focus:border-[#20729E] focus:ring-2 focus:ring-[#20729E]/20 transition-all duration-200 outline-none">
                @error('data_limite')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
            ...
            <div>
                <label for="status" class="block text-sm font-bold text-[#003262] mb-2">
                    Status
                </label>
                <select name="status" id="status"
                    ...
                    <option value="Atrasada" {{ old('status', $tarefa->status) == 'Atrasada' ? 'selected' : '' }}>Atrasada</option>
                </select>
                @error('status')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descricao" id="descricao" rows="3"
                          ...
                          placeholder="Descrição da tarefa">{{ old('descricao', $tarefa->descricao) }}</textarea>
                @error('descricao')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
